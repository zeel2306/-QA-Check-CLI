...
async function availableScript(projectPath) {
    try {
...
async function waitUntilReady(urls, process, timeout = 90_000) {
    ...
    while (Date.now() < deadline) {
        if (exited)
            throw new Error(`Local server exited with code ${exited.exitCode}: ${exited.stderr ?? ""}`);
...
export async function startLocalServer(projectPath) {
    ...
    if (!script)
        throw new Error("No start, preview, dev, or artisan server is available");
