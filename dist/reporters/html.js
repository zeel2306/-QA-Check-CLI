...
export async function writeHtmlReport(report, reportDir) {
    ...
    const gallery = screenshots
        .map((screenshot) => `<figure><img loading="lazy" src="screenshots/${escape(path.basename(screenshot))}" alt="QA screenshot"><figcaption>${escape(path.basename(screenshot))}</figcaption></figure>`)
        .join("");
    const recommendations = report.results
        .filter((result) => result.status === "FAIL" || result.status === "WARNING")
        .map((result) => `<li><strong>${escape(result.name)}:</strong> ${escape(result.message ?? "Review reported findings")}</li>`)
        .join("");
